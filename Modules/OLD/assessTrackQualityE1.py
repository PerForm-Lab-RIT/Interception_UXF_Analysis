...
def calcCalibrationVectors(sessionDictIn):
    ...
    calibDf['targetInHead_az'] = np.around(calibDf.apply(lambda row: np.rad2deg(np.arctan(row[('targetInHead','X')]
                                                                   /row[('targetInHead','Z')])),axis=1),2)

    calibDf['targetInHead_el'] = np.around(calibDf.apply(lambda row: np.rad2deg(np.arctan(row[('targetInHead','Y')]
                                                               /row[('targetInHead','Z')])),axis=1),2)

    calibDf['cycEyeInHead_az'] = np.around(calibDf.apply(lambda row: np.rad2deg(np.arctan(row[('cycEyeInHead','X')]
                                                               /row[('cycEyeInHead','Z')])),axis=1),2)

    calibDf['cycEyeInHead_el'] = np.around(calibDf.apply(lambda row: np.rad2deg(np.arctan(row[('cycEyeInHead','Y')]
                                                               /row[('cycEyeInHead','Z')])),axis=1),2)
    
    calibDf['calibErr'] = np.around(calibDf.apply(lambda row: np.rad2deg(np.arccos( np.vdot(row['cycEyeInHead'],row['targetInHead']))),
                                                axis=1,raw=True),2)
    #calibDf['subject']
...
def plotTrackQuality(sessionDictIn, saveFig = True):
    ...
    fileName = sessionDictIn['analysisParameters']['fileName']
    plt.legend()
    if(saveFig == True):
